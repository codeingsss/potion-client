(function () {
  const translations = {
    en: {
      ...
      apiNativeIntro: 'When the Native Addon is installed, .dll (Windows) / .so (Linux) / .dylib (macOS) files in <code>meteor-client/addons/</code> are loaded as shared libraries. There\'s no init/tick callback contract — the standard JNI entry point <code>JNI_OnLoad</code> is called once, immediately after loading, and you spawn threads / call the functions below from there. Unlike BeanShell/Python, this is completely unsandboxed native code, so only install addons you trust.',
      apiColJava: "Java method (PotionNative)",
      apiColJni: "JNI signature",
      apiNativeReturnNote: "Class: <code>meteordevelopment/meteorclient/scripting/PotionNative</code>. Use the JNI call function matching the return type — <code>V</code>→CallStaticVoidMethod, <code>Ljava/lang/String;</code>→CallStaticObjectMethod, <code>Z</code>→CallStaticBooleanMethod, <code>F</code>→CallStaticFloatMethod, <code>I</code>→CallStaticIntMethod, <code>D</code>→CallStaticDoubleMethod, <code>J</code>→CallStaticLongMethod. <code>set</code>/<code>command</code>/<code>commandWithArgs</code>/<code>java</code> aren't available to native addons since they take callback functions — use BeanShell/Python if you need custom modules/commands.",
    ...
    ko: {
      ...
      apiNativeIntro: "Native Addon 설치 시, <code>meteor-client/addons/</code>의 .dll(Windows)/.so(Linux)/.dylib(macOS) 파일이 공유 라이브러리로 로드됩니다. 별도의 init/tick 콜백 규약은 없고, 표준 JNI 진입점인 <code>JNI_OnLoad</code>가 로드 즉시 한 번 호출됩니다 — 여기서 원하는 대로 스레드를 만들거나 아래 함수들을 호출하면 됩니다. BeanShell/Python과 달리 샌드박스가 전혀 없는 네이티브 코드이므로, 신뢰할 수 있는 애드온만 설치하세요.",
      apiColJava: "Java 메서드 (PotionNative)",
      apiColJni: "JNI 시그니처",
      apiNativeReturnNote: "클래스: <code>meteordevelopment/meteorclient/scripting/PotionNative</code>. 반환 타입에 맞는 JNI 호출 함수를 쓰세요 — <code>V</code>→CallStaticVoidMethod, <code>Ljava/lang/String;</code>→CallStaticObjectMethod, <code>Z</code>→CallStaticBooleanMethod, <code>F</code>→CallStaticFloatMethod, <code>I</code>→CallStaticIntMethod, <code>D</code>→CallStaticDoubleMethod, <code>J</code>→CallStaticLongMethod. <code>set</code>/<code>command</code>/<code>commandWithArgs</code>/<code>java</code>는 콜백 함수를 받기 때문에 네이티브 애드온에서는 지원하지 않습니다 — 모듈/명령어가 필요하면 BeanShell/Python을 쓰세요.",
  ...
  ["downloadNote", "addonsNote", "footerBasedOn", "apiIntro", "apiCallbackCode", "apiExternalNote", "apiNativeIntro", "apiNativeReturnNote", "apiNativeBuildNote"].forEach((key) => {
